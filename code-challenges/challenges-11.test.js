...
function transformToLis(obj) {
  // Solution code here...
  return Object.entries(obj).map(letter => `<li>${letter.join(': ')}</li>`);
}
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(letter => letter.gender === 'male' || letter.gender === 'female').map(letter => letter.name).join(' and ');
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6 
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((min, next) => Number(min.height) < Number(next.height) ? min : next).name;
};
