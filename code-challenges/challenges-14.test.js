...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(value => value.match(/^((Mr.)|(Mrs.)|(Ms.)|(Dr.))\s{1}\b/));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const results = [];
  const lukesMass = parseInt(starWarsData[0].mass);
...
const sortBy = (property, arr) => {
  // Solution code here...
  if( property === 'name'){
    // Could do the same sort setup as price,
    // but localCompare is better when working with letter comparisons.
    // Incerdibly useful when letters have accents.
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const helpCheck = (rowA, colA, rowB, colB, rowC, colC) => {
    // confirms that there are 3 matching characters on the board and that none of them are blank
    return (board[rowA][colA] !== '') && (board[rowA][colA] === board[rowB][colB]) && (board[rowA][colA] === board[rowC][colC]);
  };
  // confirms that the coordinates are a standard tic-tac-toe win
